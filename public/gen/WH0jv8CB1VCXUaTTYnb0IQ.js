...
view._makeAll({
  "main.html:body": "<span>List of databases</span><select name=\"dbs\" x-bind=\"change: changeDatabase\">{{#each dbs.databases}}<option value=\"{name}\" selected=\"{equal(name, dbName)}\">{{name}}</option>{{/}}</select><div>{{#each collections}}<option value=\"{name}\">{{name}}</option>{{/}}    </div><div><input value=\"{dbName}\"/><p>{dbName}</p></div><pre><code>{{collections}}</code></pre>",
  "main.html:doctype": "",
  "main.html:footer": "",
  "main.html:header": "",
