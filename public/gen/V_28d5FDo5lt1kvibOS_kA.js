...
view._makeAll({
  "main.html:body": "<span>List of databases</span><select name=\"dbs\" x-bind=\"change: changeDatabase\">{{#each dbs.databases}}<option value=\"{{name}}\" selected=\"{equal(name, message.test.text)}\">{{name}}</option>{{/}}</select><div>{{#each collections}}<option value=\"{name}\">{{name}}</option>{{/}}    </div><div><input value=\"{message.test.text}\"/><p>{message.test.text}</p></div><pre><code>{{collections}}</code></pre>",
  "main.html:doctype": "",
  "main.html:footer": "",
  "main.html:header": "",
